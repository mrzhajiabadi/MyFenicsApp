...
upper_restriction = generate_subdomain_restriction(mesh, subdomains, 20)
lower_restriction = generate_subdomain_restriction(mesh, subdomains, 19)
                                                   

# Generate restriction corresponding to interface between the two subdomains
interface_restriction = generate_interface_restriction(mesh, subdomains, {19, 20})

# Write out for simulation import (.xml) and visualization (.xdmf)
File("mesh_upper_restriction.rtc.xml") << upper_restriction
File("mesh_lower_restriction.rtc.xml") << lower_restriction
File("mesh_interface_restriction.rtc.xml") << interface_restriction

XDMFFile("mesh_upper_restriction.rtc.xdmf").write(upper_restriction)
XDMFFile("mesh_lower_restriction.rtc.xdmf").write(lower_restriction)
